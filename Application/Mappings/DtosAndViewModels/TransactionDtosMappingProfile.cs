...
{
    public class TransactionDtosMappingProfile : Profile
    {
        public TransactionDtosMappingProfile()
        {
            CreateMap<TransactionDto, TransactionViewModel>()
                ...
                .ForMember(dest => dest.Beneficiary, opt => opt.MapFrom(src => src.Beneficiary))
                 .ForMember(dest => dest.VisualType, opt => opt.MapFrom(src => src.VisualType))
                .ReverseMap();
        }
    }
}
